...
def get_field_list(cols,tag_list):
...
def fore_data_preprocessing(forecast_df:pd.DataFrame,tag_list:list,interval:str)->(pd.DataFrame,list):
    ...
    data_fields.update({'interval':interval})  #add 'interval' tag
...
def fore_data_preprocessing_read(forecast_df,field_list,fore_horizon,interval):
    t_idx=forecast_df.index[0]
    interval_num=float(interval[:-1])
    # print(interval_num)
    result=[]
    for i in range(1,fore_horizon+1):
        result.append([ forecast_df[field_list[j]+'_'+str(i)][0] for j in range(len(field_list))])

    t_range=[t_idx+timedelta(minutes=interval_num*i) for i in range(1,fore_horizon+1)]

    forecast_df_new=pd.DataFrame(result)

    forecast_df_new.columns = field_list
    forecast_df_new.index=t_range
    return forecast_df_new