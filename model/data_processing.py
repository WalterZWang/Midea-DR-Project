...
class DataProcess():
    ...
    def other_meter_process(self):
        ...
        self.PV_meter_pro['E_diff'] = util.outlier_meter(self.PV_meter_pro, 'E_diff')
        # For battery: AE is discharging, RE is charging
        self.battery_meter_pro['Dischar_diff'] = self.battery_meter_pro['Pri_AE'].diff()
        self.battery_meter_pro['Dischar_diff'] = util.outlier_meter(self.battery_meter_pro, 'Dischar_diff')
        self.battery_meter_pro['Char_diff'] = self.battery_meter_pro['Pri_RE'].diff()
        self.battery_meter_pro['Char_diff'] = util.outlier_meter(self.battery_meter_pro, 'Char_diff')
        # dev data diff
        self.PV_dev_pro['E_diff'] = self.PV_dev_pro['EacTotal'].diff()
        self.PV_dev_pro['E_diff'] = util.outlier_meter(self.PV_dev_pro, 'E_diff')
        # For battery: AE is discharging, RE is charging
        self.battery_dev_pro['Dischar_diff'] = self.battery_dev_pro['totalDischarge'].diff()
        self.battery_dev_pro['Dischar_diff'] = util.outlier_meter(self.battery_dev_pro, 'Dischar_diff')
        self.battery_dev_pro['Char_diff'] = self.battery_dev_pro['totalCharge'].diff()
        self.battery_dev_pro['Char_diff'] = util.outlier_meter(self.battery_dev_pro, 'Char_diff')
        # MELS diff
        for key,item in self.blg_meter_pro.items():
            self.blg_meter_pro[key]['E_diff'] = self.blg_meter_pro[key]['E'].diff()
            self.blg_meter_pro[key]['E_diff'] = util.outlier_meter(self.blg_meter_pro[key], 'E_diff')
                ...
                if 'odu' in key:
                    ...
                    for co in VRF_outlier_detect_co:
                        if co in item.columns: 
                            item[co] = util.outlier_meter(item, co)
                    ...
                    for co in VRF_outlier_detect_co:
                        if co in self.VRF_data_pro[sys][key].columns:
                            self.VRF_data_pro[sys][key][co] = util.outlier_meter(self.VRF_data_pro[sys][key], co)
